...
This annex proposes approaches to extending TileMatrixSets and TileSet metadata for storing, indexing and accessing as tiles 3D, 4D and n-D (n > 2) data,
regardless of whether simple a file-based data store, a database (e.g. a GeoPackage) or a web API is used.
...
It is especially suitable for shared geo-typical models which defined only once and re-used many times, including in multiple tiles,
...
For example, vector tiles could available as usual, e.g.:

    trees/tiles/GNOSISGlobalGrid/13/5200/5715.json
    trees/tiles/GNOSISGlobalGrid/13/5200/5715.mvt

while referencing individual 3D models, available at `models/{modelId}` and textures at `textures/{textureId}`, e.g.:
...
This could either clip the geometry exactly at the tile's boundary boundary, or allow spilling onto the neighboring tile,
...
(e.g. linking to .b3dm files organized in TileMatrixSet paths).
