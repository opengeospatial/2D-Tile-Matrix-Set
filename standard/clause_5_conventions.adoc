...
NOTE: OGC APIs will use the language negotiation with HTTP headers. In this situation, it is expected that elements defined as a list of LanguageString will default into a single CharacterString that in JSON will default into a string data type. In addition in JSON encodings we are not considering namespaces or codespaces (optional in the model). This will results in a simplification of the keywords in the JSON encoding to a simple array of strings.
...
| One (mandatory)
| upperCorner | Coordinates of bounding box corner at which the value of each coordinate
...
| orderedAxis | Ordered list of names of the dimensions defined in the CRS | String | Zero or more (optional). Ordered ^c^
