...
Many recent standards emphasize the usefulness of links as a way to relate a data structure instance to other data structures and make possible navigation through resources. Essential links are make explicit in the data structures of this document (recognizable by a URI data type) but other links can be added as needed for convenience. The data structure defined here allows the addition of other links. The definition is based on the atom link as defined in section 4.2.7 of the RFC4287.

NOTE: In practice, some encodings can opt specify the essential links as part of this data structure for convenience

[#img_boundingbox-uml-model,reftext='{figure-caption} {counter:figure-num}']
.BoundingBox UML model
image::figures/AtomLink.png[BoundingBox UML model]


[#parts-of-atom-link-data-structure,reftext='{table-caption} {counter:table-num}']
.Parts of AtomLink data structure
[width = "100%",options="header"]
|===
| Names | Definition | Data type and values | Multiplicity and use
