...
The following classes are extracted from the [OGC 06-121r9] OWS Common Standard. As such, the data structures presented in this document become independent from OWS Common Standard and can be used in the emerging OGC APIs that are dependend on a common standard seting the bases for the OGC APIS called OGC API Common (avoiding a conflicting dependency on the old OWS Common, which provided a common bases for the OGC Web Services instead).
...
A basic set of data description parameters that includes human-readable title, description, and keywords is widely used in this standard and defined here as UML.
...
| description | Brief narrative description of this tile matrix set, normally available for display to a human | LanguageString data structure | Zero or more (optional) Include when available and useful
...
NOTE: OGC APIs will use language negotiation with HTTP headers. In this situation, it is expected that elements defined as a list of LanguageString will default into a single CharacterString that in JSON will default into a string data type. In JSON encodings we are not considering namespaces or codespaces (optional in the model). This will results in a simplification of the keywords in the JSON encoding to a simple array of strings.
...
A 2DBoundingBox is another type of bounding box. This type is simplified from the basic BoundingBox data type for use only with the 2D geographic coordinate reference system. This is useful to specify the extent 2D part of tile matrix set.

A WGS84BoundingBox is another type of bounding box. This type is simplified from the basic BoundingBox data type for use only with the 2D geographic coordinate reference system which uses the WGS 84 geodetic datum, where longitude precedes latitude and both are recorded in decimal degrees.
