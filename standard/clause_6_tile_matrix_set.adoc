...
| scaleDenominator | Scale denominator level of this tile matrix ^f^ | Double type | One (mandatory)
...
| minTileRow | First tile row where the coalese factor apply on this tilematrix | Non negative integer type ^b^ | One (mandatory)
| maxTileRow | Last tile row where the coalese factor apply on this tilematrix | Non negative integer type ^c^ | One (mandatory)
