...
enumeration. See <<parts-of-corner-of-origin-enum>>
 |
Zero or more (optional). Default value is "topLeft"
...
Bounding‌Box | Minimum bounding rectangle surrounding the tile matrix limits, in the supported CRS ^b^ | BoundingBox data structure, see <<parts-of-boundingbox-data-structure>> | Zero or one (optional)
...
^b^     In the same CRS than the TileMatrixSet. boundingBox SHOULD NOT be used to calculate the position of the tiles in the CRS space; please use cornerOfOrigin and pointOfOrigin of the corresponding TileMatrix instead. (see <<tilematrixsetlimits2d-requirements-class>>).
|===
 
...
AccessConstraints | Restrictions on the availability of the Tile Set that the user needs to be aware of before using or redistributing the Tile Set | ClassificationCode data structure, see <<parts-of-classification-code-enum>> | Zero or one (optional)

| dataType

DataType | Type of data represented in the tiles | CharacterString. Examples are "features" or "coverages" | Zero or one (optional)

| mediaType

MediaType | Media types available for the tile set | CharacterString restricted by RFC6838 section 4.2 | Zero or more (optional)
...
Tile‌Matrix‌Set‌Limits | Index limits for this tileMatrixSet | TileMatrixSetLimits data structure, see <<parts-of-tilematrixsetlimits-data-structure>> | Zero or more (optional) Should be include when the boundary of the data is a fragment of the boundary of the tileMatrixSet ^a,b^
...
Link | links to related resources | AtomLink data structure, see <<parts-of-atom-link-data-structure>>.

Common values of the rel attribute are: dataset, tiles, alternate ^e^ | Zero or more (optional)
...
A Layer is a set of geographic objects of the same type together in a way that can be presented to the user. Its elements are defined with a data structure defined in <<parts-of-geospatial-data-data-structure>>.
...
DataType | Type of data represented in the layer | CharacterString. Examples are "features" or "coverages" | Zero or one (optional)

| geometryType

GeometryType | The geometry type of the features shown in this layer | CharacterString^b^ | Zero or more (optional)

| featureType

FeatureType | Feature type identifier | CharacterString | Zero or one (optional)
...
Theme | Category where the layer can be group | CharacterString | Zero or more (optional)
...
Style | Style used to represent the layer in the tileset | Style data structure, see <<parts-of-style-data-structure>> | Zero or one (optional)

| property

Property | properties represented by the features in this layer | FeatureAttribute data structure. See <<parts-of-feature-attribute-data-structure>> | Zero or more (optional)

| link

Link | links to related resources | AtomLink data structure, see <<parts-of-atom-link-data-structure>>.

Common values of the rel attribute are: collection ^h^ | Zero or more (optional)

4+|^a^    The multilingual scoping rules in <<multilingualTextEncoding>>>> apply

^b^     Geometry names as defined in ISO 19107 such as GM_Point, GM_Curve, GM_Polygon
...
^h^     'collection' relation should point to a URL to get a collection represented in this layer.
|===
...
Name | identifier of the property | CodeType, as adaptation of MD_Identifier class ISO 19115 | One (mandatory)
| type

Type | The data type of the property | CharacterString | One (mandatory)

| pattern

Pattern | Regular expression to validate the values of the property | CharacterString | Zero or one (optional)

| mediaType
MediaType | Encodings of a complex property (e.g. an image) | CharacterString restricted by RFC6838 section 4.2 | Zero or one (optional)

| acceptedValues

AcceptedValues | Valid values of the property | CharacterString | Zero or more (optional)

| range

Range
| Range of valid values expressed as an array of two items | CharacterString | Zero or two (optional)

| multiplicity
Multiplicity | Multiplicity of the property | CharacterString | Zero or one (optional)
...
| observedPropertyURI
ObservedPropertyURI | measured phenomenon (variable) URI pointing to a representation of the definition of the phenomenon | URI | Zero or one (optional)

| uom

UoM | Units of measure characterizing the values of the property | CharacterString | Zero or one (optional)
4+|
^a^     The multilingual scoping rules in <<multilingualTextEncoding>>>> apply.
|===
...
CreatedOn | Timestamp when the Tile Set was first produced | DateTime | Zero or one (optional)

| publishedOn

PublishedOn | Timestamp when the Tile Set was first made available to the users | DateTime | Zero or one (optional)
...
Style | identifier of a style | CodeType, as adaptation of MD_Identifier class ISO 19115 | Zero or one (optional)

| link

Link | links to related resources | AtomLink data structure, see <<parts-of-atom-link-data-structure>>.

Common values of the rel attribute are: style and styleSpec ^a^ | Zero or more (optional)
4+|
^a^     'style' relation should point to a URL to get the style description, 'styleSpec' relation should point to a URL for the specification or standard used to define the style.
|===
...
Coordinate | Location of the center point of the tile set | GM_Point data structure | one (mandatory)
| tileMatrix
