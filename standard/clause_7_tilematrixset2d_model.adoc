...
^f^    In previous versions was called topLeftCorner and cornerOfOrigin did not exist
...
DataType | Type of data represented in the tiles | DataTypeCode code list | Zero or one (optional)
...
Tile‌Matrix‌Set‌Limits | Index limits for this tileMatrixSet | TileMatrixSetLimits data structure, see <<parts-of-tilematrixsetlimits-data-structure>> | Zero or more (optional) Should be include when the boundary of the data is a fragment of the boundary of the tileMatrixSet ^g^
...
Style | Style used to generate the tiles in the tileset | Style data structure, see <<parts-of-style-data-structure>> | Zero or one (optional)
...
Link | Links to related resources | AtomLink data structure, see <<parts-of-atom-link-data-structure>>.^e^ | Zero or more (optional)
...
^g^     If missing, there are no limits other that the ones imposed by the TileMatrixSet.
...
DataType | Type of data represented in the layer | DataTypeCode code list | Zero or one (optional)
...
Link | Links to related resources | AtomLink data structure, see <<parts-of-atom-link-data-structure>>. ^h^ | Zero or more (optional)
...
Identifier | identifier of the property | CodeType, as adaptation of MD_Identifier class ISO 19115 | One (mandatory)
| type
...
UoMURI | URI pointing to a representation of the definition of the units of measure characterizing the values of the property | CharacterString | Zero or one (optional)
4+|
^a^     The multilingual scoping rules in <<multilingualTextEncoding>>>> apply.
|===
...
Link | Links to style related resources | AtomLink data structure, see <<parts-of-atom-link-data-structure>>. ^b^ | Zero or more (optional)
4+|^a^    The multilingual scoping rules in <<multilingualTextEncoding>>>> apply
  |
...
|===
| Names | Definition
| photograph | Images with values representing colors
| geometries | Features composed by geometries and properties
