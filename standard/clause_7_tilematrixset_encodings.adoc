...
The JSON encoding has been done respecting the naming and types of the original classes. However, there are some differences.  <<class-attributes-JSON-element-equivalences>> describes some exceptions in the mapping between the class attributes and corresponding JSON element.
