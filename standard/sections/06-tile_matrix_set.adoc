...
| identifier
| Tile matrix set identifier{blank}footnote:g6[TileMatrixSet identifies
...
The (https://docs.opengeospatial.org/pol/09-046r5.html#_naming_rule)[OGC Naming
Authority - Procedures document] specifies rules to transform form URN to URI.
Implement a on-the-fly translations based on these rules is possible.]
| Zero or one (optional)
