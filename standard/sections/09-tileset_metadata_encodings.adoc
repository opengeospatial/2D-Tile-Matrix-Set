...
The encoding of PropertySchema adopts the JSON Schema form starting by and element
propertiesSchema that defines the GeoJSON element "properties" for this layer (the
composition of "properties" is undefined by GeoJSON but it can be define for a layer
